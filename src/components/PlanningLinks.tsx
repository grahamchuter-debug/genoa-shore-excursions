import Link from "next/link";

const LINKS = [
  { href: "/shore-excursions", label: "Shore Excursions" },
  { href: "/guides", label: "Planning Guides" },
  { href: "/cruise-port-guide", label: "Genoa Cruise Port Guide" },
  { href: "/cruise-planner", label: "Italian Riviera Cruise Planner" },
  { href: "/ship-schedules/genoa", label: "Ship Schedules" },
  { href: "/compare/portofino-vs-camogli", label: "Compare" },
  { href: "/faq", label: "FAQ" },
];

export function PlanningLinks({ heading = "Keep planning your Italian Riviera cruise" }: { heading?: string }) {
  return (
    <section className="rounded-2xl border border-coastal-100 bg-coastal-50/60 p-6 sm:p-8">
      <h2 className="font-display text-xl font-semibold text-gray-900">{heading}</h2>
      <div className="mt-4 flex flex-wrap gap-2">
        {LINKS.map((l) => (
          <Link key={l.href} href={l.href} className="pill hover:bg-coastal-100 transition-colors">
            {l.label}
          </Link>
        ))}
      </div>
    </section>
  );
}
